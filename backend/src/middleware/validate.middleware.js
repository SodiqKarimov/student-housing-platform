...
const userValidation = {
  create: [
    ...
    body('email').notEmpty().withMessage('Email kiritilishi shart').isEmail().withMessage("Email noto'g'ri formatda").normalizeEmail(),
    body('password').notEmpty().withMessage('Parol kiritilishi shart').isLength({ min: 8 }).withMessage("Parol kamida 8 ta belgidan iborat bo'lishi kerak"),
    body('role').notEmpty().withMessage('Rol kiritilishi shart').isIn(['SUPER_ADMIN', 'ADMIN', 'DEAN_OFFICE', 'DORMITORY_STAFF', 'TUTOR']).withMessage("Rol noto'g'ri"),
    phoneRule('phone'),
    validate,
  ],
};
...
const dormitoryValidation = {
  create: [
    ...
    body('gender').notEmpty().withMessage('Jins kiritilishi shart').isIn(['MALE', 'FEMALE', 'MIXED']).withMessage("Jins noto'g'ri"),
    body('totalCapacity').notEmpty().withMessage('Sig\'im kiritilishi shart').isInt({ min: 1 }).withMessage("Sig'im musbat son bo'lishi kerak"),
